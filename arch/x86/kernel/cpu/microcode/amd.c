...
static bool __verify_patch_section(const u8 *buf, size_t buf_size, u32 *sh_psize)
{
	...
	if (p_size > buf_size) {
	        pr_debug("Patch of size %u too long.\n", p_size);
    		return false;
    	}
...
static size_t parse_container(u8 *ucode, size_t size, struct cont_desc *desc)
{
	...
	if (hdr > size - CONTAINER_HDR_SZ) {
		pr_debug("Equivalence table truncated.\n");
		return 0;
	}
        
	table.num_entries = hdr / sizeof(struct equiv_cpu_entry);
        
        /* Equivalence table starts immediately after the container header. */
	...
	eq_id = find_equiv_id(&table, bsp_cpuid_1_eax);
        ...
        /* Advance buf and size past the equivalence table. */
	buf  += hdr + CONTAINER_HDR_SZ;
	size -= hdr + CONTAINER_HDR_SZ;
